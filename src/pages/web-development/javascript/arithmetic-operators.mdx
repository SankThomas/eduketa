...
In the second case however, because the string value is the one that begins, then the entire expression is treated as a string. So, even `x` and `y` in the second expression are considered strings. Therefore, the values are concatenated and not added. Read more about [String Concatenation](/web-development/javascript/strings).
